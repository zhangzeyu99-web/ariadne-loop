...
- Tightened Brief: Run a stateful loop around "Prepare a public release without shipping broken docs, stale generated examples, or an unverified tag": inspect real context, take the smallest useful action, verify each gate, persist evidence, then stop, continue, rollback, or ask for human confirmation before changing external state.
...
- human-required effects: git tag, GitHub release
...
- Apply the execution policy before external-impact actions: git tag, GitHub release
- Ask for confirmation before human-required effects: git tag, GitHub release
...
- Always ask before: git tag, GitHub release.
